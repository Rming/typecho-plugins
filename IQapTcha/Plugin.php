 ...
 * 1.1.0 at 2012-07-12
 * 1、增加博主可以不需要解锁的功能，可以自由设置
 * 2、增加解锁后立即自动提交评论的功能，可以自由设置
 * 3、后台设置区增加一些必要的检验
 * 
 * 安装方法：请先禁用之前的版本，然后上传覆盖激活
 * 
 * 1.0.0 at 2012-07-10
...
class IQapTcha_Plugin implements Typecho_Plugin_Interface
{
	...
	{
		//jquery 设置
		$opt_jq_set = new Typecho_Widget_Helper_Form_Element_Radio('opt_jq_set', array('0'=> '自己处理', '1'=> '自动载入'), 1, 'jQuery 来源', '如果选择 【自动载入】，会从 Google API 自动载入 jQurey 1.4.4 到 header<br />http://ajax.googleapis.com/ajax/libs/jquery/1.4.4/jquery.min.js');
...
EOT;
    ...
    public static function filter($comment, $post)
    {
		...
		if ($iQapTchaOpt->opt_ip != "none") {
			if (IQapTcha_Plugin::check_ip($iQapTchaOpt->words_ip, $comment['ip'])) {
		...
		if ($iQapTchaOpt->opt_ban != "none") {
			if (IQapTcha_Plugin::check_in($iQapTchaOpt->words_ban, $comment['text'])) {
		...
		if ($iQapTchaOpt->opt_chk != "none") {
			if (IQapTcha_Plugin::check_in($iQapTchaOpt->words_chk, $comment['text'])) {
